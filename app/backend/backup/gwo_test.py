...
class GWOLogAnalyzer:
    """
    Grey Wolf Optimizer for security log analysis.
    
    This implementation uses the Grey Wolf Optimizer to detect security anomalies
    in log data. The approach models the social hierarchy and hunting behavior of
    grey wolves to find optimal parameters for anomaly detection:
    - Alpha wolf: Best solution (best anomaly detection configuration)
    - Beta wolf: Second best solution
    - Delta wolf: Third best solution
    - Omega wolves: Remaining solutions that follow the top three
    """
    ...
    def __init__(self,
        ...
        """
        Initialize the GWO log analyzer.
        
        Args:
            n_wolves: Number of wolves in the pack
            max_iterations: Maximum number of iterations
            a_init: Initial value of parameter 'a' (decreases linearly to 0)
            domains: Parameter domains (defaults to [0,1] for all dimensions)
        """
        self.n_wolves = n_wolves
        self.max_iterations = max_iterations
        self.a_init = a_init
        
        # Search space dimensions (features in our solution)
        self.dimensions = 18
        self.domains = domains or [(0, 1) for _ in range(self.dimensions)]
        
        # Security indicators for threat detection
    ...
    def analyze(self, logs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Analyze logs using Grey Wolf Optimizer to detect security anomalies.
        ...
        # Track progress of the hunt
        hunting_progress = []
        
        # Main GWO loop
        for iteration in range(self.max_iterations):
            # Update the control parameter 'a' (linear decrease)
            ...
            for i in range(self.n_wolves):
                for j in range(self.dimensions):
                    # Calculate parameters for alpha, beta, delta
                    ...
                    # Distance from current wolf to leader wolves
                    D_alpha = abs(C1 * alpha_pos[j] - positions[i, j])
                    D_beta = abs(C2 * beta_pos[j] - positions[i, j])
                    D_delta = abs(C3 * delta_pos[j] - positions[i, j])
                    
                    # Movement components toward leader wolves
                    X1 = alpha_pos[j] - A1 * D_alpha
                    X2 = beta_pos[j] - A2 * D_beta
                    X3 = delta_pos[j] - A3 * D_delta
                    
                    # Update position based on leaders' influence
        ...
        # Use alpha position (best solution) for final analysis
        threat_scores = self._calculate_threat_scores(alpha_pos, features)
        
        # Identify threats based on threshold
        threshold = 0.65
        ...
        for idx in threat_indices:
            ...
            # Calculate contributions of each feature to the threat score
        ...
        # Identify top vulnerabilities based on feature weights
    ...
    def _extract_features(self, logs: List[Dict[str, Any]]) -> Tuple[np.ndarray, List[int]]:
        """
        Extract features from logs for GWO analysis.
        
        Features cover various security aspects:
        - Location-based risk
        - Network indicators (IPs, ports, protocols)
        ...
        for i, log in enumerate(logs):
            # Skip logs with minimal information
            ...
            # 11. External source IP (non-RFC1918)
            ...
            # 12. External destination IP (non-RFC1918)
            ...
            if timestamp:
                try:
                    ...
                    # Consider 8pm-6am as off-hours
                    if dt.hour < 6 or dt.hour >= 20:
                        feature_vector[15] = 0.5
                except (ValueError, TypeError):
                    pass
            
            # 17. Admin/root/system username
            ...
            # Only include logs with at least one feature
    ...
    def _evaluate_fitness(self, position: np.ndarray, features: np.ndarray) -> float:
        """
        Evaluate the fitness of a wolf position.
        
        A good position should:
        - Effectively identify security threats
        - Maintain good separation between normal and anomalous events
        - Not flag too many events as threats (avoid false positives)
        """
        if features.shape[0] == 0:
            return 0.0
            
        # Calculate threat scores using position weights
        ...
        # Calculate skewness (positive skew is good - indicates few high scores)
        skewness = 0.0
        if len(scores) > 2 and std_dev > 0:
            skewness = np.mean(((scores - mean) / std_dev) ** 3)
        
        # Calculate proportion of high-scored events (should be small)
        high_threshold = 0.7
        high_ratio = np.mean(scores > high_threshold)
        
        # Ideal ratio: Between 5-10% flagged as threats
        ratio_score = 0.0
        if high_ratio > 0:
            if high_ratio <= 0.1:
        ...
        # Feature weight distribution (critical features should have higher weights)
    ...
    def _calculate_threat_scores(self, position: np.ndarray, features: np.ndarray) -> np.ndarray:
        """Calculate threat scores for all logs using the best wolf position"""
        # Calculate scores
        scores = np.dot(features, position)
    ...
    def _calculate_feature_contributions(self, position: np.ndarray, feature_vector: np.ndarray) -> List[Tuple[int, float]]:
        """Calculate the contribution of each feature to the threat score"""
        # Calculate individual contributions: weight * feature
    ...
    def _describe_threat_indicators(self, feature_contributions: List[Tuple[int, float]], log: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Convert feature contributions to human-readable threat indicators"""
    ...
    def _get_indicator_detail(self, feature_idx: int, log: Dict[str, Any]) -> str:
        """Get specific details for a threat indicator"""
    ...
    def _identify_top_vulnerabilities(self, position: np.ndarray, logs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Identify top vulnerability categories based on feature weights"""
        # Get indices of top weighted features
        top_indices = np.argsort(position)[::-1][:5]  # Top 5 features
        
        # Group feature indices into categories
        categories = {
        ...
        # Find example logs for each top category
        vulnerabilities = []
    ...
    def _get_recommendation(self, category: str) -> str:
        """Get security recommendation based on vulnerability category"""
        recommendations = {
    ...
    def _calculate_overall_threat(self, threat_scores: np.ndarray, detected_threats: List[Dict[str, Any]]) -> float:
        """Calculate overall threat level for the log set"""
        if len(threat_scores) == 0:
            return 0.0
            
        # Factor 1: Maximum threat score
        max_threat = np.max(threat_scores) if threat_scores.size > 0 else 0
        
        # Factor 2: Percentage of high threat events (above 0.7)
        threat_percentage = np.mean(threat_scores > 0.7)
        threat_factor = min(1.0, threat_percentage * 5)  # Cap at 1.0
        
        # Factor 3: Severity of detected threats
        severity_factor = 0.0
        if detected_threats:
            # Critical events have more impact
            critical_events = [
            ...
            # Multiple threats indicate a more severe situation
            if len(detected_threats) >= 3:
                severity_factor += 0.3
                
            # High scores indicate more confident detections
